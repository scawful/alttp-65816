import {CompletionItem, CompletionItemKind, Location, MarkupContent, MarkupKind, Position, SymbolInformation, SymbolKind, TextDocument} from 'vscode-languageserver'
...
export function resolveDefinitions(pos: Position, doc: TextDocument):
    Location[] {
      ...
      do {
        ...
        if (m == null) break;
        locations.push({match: m[0], index: m.index})
      } while (m);
      if (!locations[0]) {
        return []
      };

      var searchable = TextDocument.create('test', 'asm', 0, disassembly)
...
- 363, character: 0
      ...
      */
      ...
      return locationsMapped.map((d) => {
        return {
          uri: 'disassembly://Disassembly.asm', range: {
...
export function hoverHandler(pos: Position, doc: TextDocument):
    string|MarkupContent {
      ...
      matches = matches.filter((d) => {
        return d.index < pos.character &&
        pos.character < (d.index + d.match.length)
      })
      ...
      if (match.match(ramRegExp) != null) {
        if (match.length == 5) {
          match = '$7E' + match.substr(1, 4);
        }


        var ramMatch = allMemory.find(
            (r) => r.beginsAt <= parseInt('0x' + match.substr(1, 6)) &&
                r.endsAt >= parseInt('0x' + match.substr(1, 6)))
...
export function resolveSymbols(
    uri: string, doc: TextDocument): SymbolInformation[] {
  var text = doc.getText(
      {start: {line: 0, character: 0}, end: {line: 999999, character: 999999}})
  ...
  do {
    ...
    matches.push({match: m[0], index: doc.positionAt(m.index)})
  } while (m);
  ...
  for (var i = 0; i < matches.length; i++) {
    ...
    symbols.push({
      ...
      location: {range: {start: matches[i].index, end: endIndex}, uri: doc.uri}
    });
  }
